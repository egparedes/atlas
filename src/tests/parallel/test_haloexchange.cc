...
void test_rank2( Fixture& f ) {
    ...
    for ( int p = 0; p < f.N; ++p ) {
        for ( size_t i = 0; i < 3; ++i ) {
            arrv( (size_t)p, i, (size_t)0 ) = ( size_t( f.part[p] ) != mpi::comm().rank() ? 0 : -f.gidx[p] * std::pow( 10, i ) );
...
void test_rank2_l1( Fixture& f ) {
    ...
    for ( int p = 0; p < f.N; ++p ) {
        for ( size_t i = 0; i < 3; ++i ) {
            arrv_t( (size_t)p, i, (size_t)0 ) = ( size_t( f.part[p] ) != mpi::comm().rank() ? 0 : -f.gidx[p] * std::pow( 10, i ) );
            arrv_t( (size_t)p, i, (size_t)1 ) = ( size_t( f.part[p] ) != mpi::comm().rank() ? 0 : f.gidx[p] * std::pow( 10, i ) );
        }
    }
    arr_t.syncHostDevice();
...
void test_rank2_paralleldim2( Fixture& f ) {
    ...
    for ( int p = 0; p < f.N; ++p ) {
        for ( size_t i = 0; i < 3; ++i ) {
            arrv( i, (size_t)p, (size_t)0 ) = ( size_t( f.part[p] ) != mpi::comm().rank() ? 0 : -f.gidx[p] * std::pow( 10, i ) );
...
void test_rank1_cinterface( Fixture& f ) {
    ...
    atlas__HaloExchange__execute_strided_double( &( f.halo_exchange ), arrv.data(), &( strides[1] ), &( shapes[1] ),
                                                 1 );
