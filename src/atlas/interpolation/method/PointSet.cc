...
PointSet::PointSet( Mesh& mesh ) {
    ...
    if ( fastBuildKDTrees ) {
        ...
        for ( size_t ip = 0; ip < npts_; ++ip ) {
            pidx.emplace_back( PointIndex3::Point( coords( ip, (size_t)0 ), coords( ip, (size_t)1 ), coords( ip, (size_t)2 ) ), ip );
        }
        ...
        for ( size_t ip = 0; ip < npts_; ++ip ) {
            tree_->insert(
                PointIndex3::Value( PointIndex3::Point( coords( ip, (size_t)0 ), coords( ip, (size_t)1 ), coords( ip, (size_t)2 ) ), ip ) );
        }
    }
}
